...
- 大模型 API 配置：在网页右侧填写 provider、model、base URL、API key，优先保存到本地 `config/llm.local.json`；如果项目目录没有写入权限，会自动保存到用户目录 `.lif-memory/llm.local.json`，并可直接测试连接。
...
更方便的方式是在网页右侧“大模型 API”卡片中填写并保存。项目目录不可写时，系统会自动 fallback 到用户目录 `.lif-memory/llm.local.json`。这些本地配置文件不会上传到 GitHub。
