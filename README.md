...
原始笔记仍然保存在 Obsidian 中。LIF-Memory 只保存派生出来的场状态、MemoryEvent、会话、反馈、embedding 缓存、AhaCard 和调参结果。

核心边界：

```text
Raw history is not memory.
MemoryEvent is the compressed state that can be reactivated.
Spike is not insight.
Spike only opens the gate for insight reconstruction.
```

也就是说，MemoryEvent 负责把历史压缩成可激活状态，LIF 负责判断“什么时候该重构”，AhaEngine 负责生成“到底想通了什么”。
...
| Dynamic MemoryEvent | 将原始 Markdown / JSON / 会话压缩成 `topic / claim / problem / decision / evidence / emotion / importance / status / confidence`，并在新问题下重新激活。 |
...
| 冲突状态检测 | 识别 EEG vs ECG、稀疏 vs 异步、实现 vs 洞察、检索 vs 重建等路线冲突，避免旧记忆直接污染新判断。 |
...
## Dynamic MemoryEvent / 动态记忆事件层

Dynamic MemoryEvent 是 LIF-Memory 的长期记忆单元。它解决的问题不是“把更多历史塞进 prompt”，而是把分散历史压缩成可以被新问题重新激活的状态。
...
claim       当时形成了什么判断
problem     当时卡在哪里
decision    当时有没有形成下一步动作
evidence    支撑这个判断的来源片段
emotion     anxious / blocked / decisive / curious / stable / neutral
importance  1-5 的重要性
status      active / uncertain / outdated / contradicted
confidence  置信度
source      来源文件或会话位置
```
...
```text
普通 RAG：query -> similar chunks -> answer
LIF-Memory：query -> activate MemoryEvents -> reconstruct current state -> insight card / answer
```

也就是说，系统不是简单查“像不像”，而是判断：
...
该文件可能包含私人笔记派生记忆，不应该提交到 GitHub。

### 当前内置冲突类型

| conflict | meaning |
| --- | --- |
| `EEG_vs_ECG` | EEG 脑电主线与 ECG 心电降难度路线之间的方向冲突。 |
| `Sparse_vs_Async` | 稀疏性证明与异步性证明之间的论证焦点冲突。 |
| `Build_vs_Insight` | 做实验 / 写代码与提炼本质洞察之间的行动冲突。 |
| `Recall_vs_Reconstruct` | 静态检索 / 摘要与动态激活 / 重建之间的记忆系统路线冲突。 |
...
lif_memory_event_engine.py --json-output    MemoryEvent activation packet
...
| LIF 联系场 | 笔记 -> MemoryEvent + 稀疏语义 + dense 语义 + 图扩散 + LIF 电压 | 能解释来源，能保留长期问题压力，能结合反馈、冲突状态和神经元状态 | 不是高性能向量库，embedding 部分仍需要模型质量支撑 |
...
`dense_similarity` 负责语义召回，`sparse_similarity` 负责可解释性。MemoryEvent 负责保存可激活的长期状态，LIF 电压和图扩散负责把“单次相似”提升为“长期问题场”。
...
| `lif_memory_event_engine.py` | 动态 MemoryEvent 层：抽取结构化记忆、激活旧状态、检测冲突、生成 insight card。 |
...
| `docs/` | 架构、relation spike、连续问题场、Dynamic MemoryEvent、AhaEngine、后续路线文档。 |
...
```text
lif_sessions.json / lif_conclusions.json -> MemoryEvent -> activated memories -> insight cards
```
...
```text
lif_spikes.json + memory activation packet + high_loss observations -> AhaEngine -> lif_aha_cards.json / AhaCards.md
```
...
lif_memory_events.json
lif_dynamic_memory_packet.json
LIF-Dynamic-Memory-Report.md
lif_aha_cards.json
```

它们可能包含私人笔记派生信息、本地模型路径、会话历史、反馈历史、MemoryEvent、AhaCard 或 API 配置。
...
它目前是一个本地、可审计的记忆场原型：用 MemoryEvent 压缩历史状态，用 LIF 状态负责触发，用 embedding 提高召回，用图扩散恢复联系，用冲突状态阻止旧记忆污染新判断，用 AhaEngine 做解释重构，用 LLM 改善解释，用人工反馈更新控制器。
