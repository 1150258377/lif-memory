...
STOP_TERMS = {
    "今天", "现在", "这个", "那个", "就是", "然后", "需要", "应该", "感觉", "什么", "一个",
    "自己", "进行", "目前", "接下来", "the", "and", "with", "that", "this", "from",
}
...
class MemorySlot:
    ...
    def label(self) -> str:
        ...
        return self.slot_id if not terms else f"{self.slot_id}: " + " / ".join(terms)
...
def tokenize(text: str) -> list[str]:
    tokens: list[str] = []
    tokens.extend(match.group(0).lower() for match in EN_WORD_RE.finditer(text))
...
def normalize_vector(values: Sequence[float]) -> list[float]:
    ...
    return [0.0 for _ in values] if norm <= 0 else [value / norm for value in values]
...
def observation_intensity(features: PrimitiveFeatures, term_count: int) -> float:
    ...
    base += 0.28 * features.actionability + 0.35 * features.blocker + 0.22 * features.specificity
...
def choose_slot(observation: Observation, slots: Sequence[MemorySlot], min_similarity_for_existing: float) -> tuple[MemorySlot, float]:
    ...
    best_slot, best_similarity = max(
        ((slot, cosine(observation.embedding, slot.prototype)) for slot in occupied),
...
def update_slot(slot: MemorySlot, observation: Observation, similarity: float, learning_rate: float) -> Assignment:
    ...
    slot.reconstruction_loss = reconstruction_loss if slot.assigned_count == 1 else 0.78 * slot.reconstruction_loss + 0.22 * reconstruction_loss
    ...
    return Assignment(observation=observation, slot_id=slot.slot_id, similarity=similarity, reconstruction_loss=reconstruction_loss)
...
def reconstruct_unsupervised_memory_field(
    ...
    slots = [MemorySlot(slot_id=f"slot_{idx}", prototype=[0.0] * dim) for idx in range(max(1, slot_count))]
    if not observations:
        return UnsupervisedMemoryFieldResult(VERSION, slots, [], 1.0, [], threshold)

    # Keep one evidence pass in the returned field. The argument `epochs` is accepted
    # for CLI stability, but this deterministic online version uses a single
    # reconstruction pass so evidence and voltage are not counted multiple times.
    ...
    for observation in observations:
        ...
        assignments.append(update_slot(slot, observation, similarity, learning_rate))
    ...
    return UnsupervisedMemoryFieldResult(VERSION, slots, assignments, global_loss, high_loss, threshold)
...
def render_markdown(result: UnsupervisedMemoryFieldResult) -> str:
    ...
    for slot in result.slots:
        if slot.assigned_count <= 0:
            continue
        lines.extend([f"### {slot.label}", "", f"- 电位：`{slot.v:.2f}`，重建误差：`{slot.reconstruction_loss:.2f}`", "- 证据："])
    ...
    for item in result.high_loss_observations:
        lines.append(f"- loss=`{item.reconstruction_loss:.2f}` slot=`{item.slot_id}` `{item.observation.day.isoformat()}` {item.observation.text}")
    lines.extend([
        "",
        "## 使用含义",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--epochs", type=int, default=2, help="Accepted for CLI stability; current version uses one deterministic evidence pass.")
...
def main() -> None:
    ...
    result = reconstruct_unsupervised_memory_field(notes, slot_count=args.slots, epochs=args.epochs, fallback_day=today)
